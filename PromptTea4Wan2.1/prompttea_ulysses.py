...
# This script is modified based on https://github.com/MingXiangL/Teacache-xDiT/blob/main/teacache/wan21_teacache.py.
import argparse
...
def calculate_PCA_threshold(reference_embeddings, context, t1=0.09, t2=0.23):
    """
...
def t2v_generate(self,
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            if self.reference_embeddings != None:
                threshould = calculate_PCA_threshold(self.reference_embeddings, context)
                arg_c = {'context': context, 'seq_len': seq_len, 'teacache_threshould': threshould}
                arg_null = {'context': context_null, 'seq_len': seq_len, 'teacache_threshould': threshould}
            else:
...
def prompttea_forward(
    ...
    teacache_thresh = teacache_threshould if teacache_threshould != None else self.teacache_thresh
    if self.enable_teacache:
        ...
        if self.cnt%2==0: # even -> conditon
            ...
            if self.cnt < self.ret_steps or self.cnt >= self.cutoff_steps:
                ...
                delta_e0 = ((modulated_inp-self.previous_e0_even).abs().mean() / self.previous_e0_even.abs().mean()).cpu().item()
                x_and_t = create_poly_features(np.array([delta_e0]), np.array([self.cnt//2]))
                l1 = self.fit_model.predict(x_and_t)
                self.accumulated_rel_l1_distance_even += l1
            ...
            if self.cnt < self.ret_steps or self.cnt >= self.cutoff_steps:
                ...
                self.accumulated_rel_l1_distance_even = 0
                # 新增：初始化 should_calc_cfg_odd
                should_calc_cfg_odd = True  # 或根据逻辑设置为 False
                self.accumulated_cfg_l1_distance_odd = 0  # 同步重置累积值
                # 确保所有GPU做出相同决定
                if dist.is_initialized():
                    tensor_should_compute = torch.tensor(should_calc_odd, device=device)
                    dist.broadcast(tensor_should_compute, src=0)
                    should_calc_odd = tensor_should_compute.item()
                    # 新增：同步 should_calc_cfg_odd
                    tensor_should_compute_cfg = torch.tensor(should_calc_cfg_odd, device=device)
                    dist.broadcast(tensor_should_compute_cfg, src=0)
                    should_calc_cfg_odd = tensor_should_compute_cfg.item()
            else:
                # 原逻辑保持不变
                delta_e0 = ((modulated_inp-self.previous_e0_odd).abs().mean() / self.previous_e0_odd.abs().mean()).cpu().item()
                x_and_t = create_poly_features(np.array([delta_e0]), np.array([self.cnt//2]))
                l1 = self.fit_model.predict(x_and_t)
                self.accumulated_rel_l1_distance_odd += l1
                if dist.is_initialized():
                ...
                if self.accumulated_rel_l1_distance_odd < teacache_thresh:
                    ...
                    self.accumulated_rel_l1_distance_even = 0
                # 判断是否跳过cfg    
                cfg_l1 = self.cfg_codebook.get(((self.cnt - 1) // 2), 0)
                self.accumulated_cfg_l1_distance_odd += cfg_l1
                if self.accumulated_cfg_l1_distance_odd < self.cfgcache_thresh:
                    should_calc_cfg_odd = False
                else:
                    should_calc_cfg_odd = True
                    self.accumulated_cfg_l1_distance_odd = 0
                # 确保所有GPU做出相同决定
                if dist.is_initialized():
                    tensor_should_compute = torch.tensor(should_calc_odd, device=device)
                    dist.broadcast(tensor_should_compute, src=0)
    ...
    if self.enable_teacache: 
        if self.is_even:
            ...
            if not should_calc_odd:
                ...
                ori_x = x.clone()
                if should_calc_cfg_odd:
                    for block in self.blocks:
                        x = block(x, **kwargs)
                else:
                    # print(f"第{self.cnt}步使用cfgcache--------------------------------")
...
def usp_prompttea_forward(
    ...
    if self.enable_teacache:
        ...
        if self.is_even:
            ...
            # 确保所有GPU做出相同决定
            if dist.is_initialized():
                tensor_should_compute = torch.tensor(should_calc_even, device=device)
                dist.broadcast(tensor_should_compute, src=0)
                should_calc_even = tensor_should_compute.item()
            # 更新
            self.previous_e0_even = modulated_inp.clone()
        else:
            should_calc_cfg_odd = True
        ...
        if self.is_even:
            ...
            if not should_calc_odd:
                ...
                # 通过模型块传递
                    ...
                    chunked_x = chunked_x + torch.chunk(self.previous_residual_even, sp_world_size, dim=1)[sp_rank]
                # 计算局部残差
                local_residual = chunked_x - original_x
                # 收集所有GPU上的残差，构建完整残差
                if dist.is_initialized():
...
def generate(args):
    ...
    if args.ulysses_size > 1 or args.ring_size > 1:
        new_forward_func = usp_prompttea_forward
        logging.info(f'Use usp_prompttea_forward')
    else:
        new_forward_func  = prompttea_forward
        logging.info(f'Use prompttea_forward')
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        codebook = prepare_codebook(args.codebook_dir)
        codebook_dict = {(i): value for i, value in zip(codebook.index.get_level_values(0),
                                                        codebook['output_value'])}
        # cfgcache
        wan_model.__class__.cfgcache_thresh = args.cfgcache_thresh
        wan_model.__class__.accumulated_cfg_l1_distance_odd = 0
        wan_model.__class__.cfg_codebook = codebook_dict

        # Prompt_Complexity_Aware
        wan_t2v.reference_embeddings = None
        if args.use_pca:
            reference_embeddings = torch.load("reference_embeddings_wan2.1.pt")
        ...
        if args.use_ret_steps:
            ...
            if '14B' in args.ckpt_dir:
                ...
                wan_model.__class__.fit_model = joblib.load("fit_model_wan2.1.pkl")
            ...
            if '14B' in args.ckpt_dir:
                ...
                wan_model.__class__.fit_model = joblib.load("linear_regression_model.pkl")
            wan_model.__class__.ret_steps = 1*2
